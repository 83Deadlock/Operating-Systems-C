...
int main(int argc, char* argv[]){
	...
	if(mkfifo("pipe_cliente_servidor",0666) == -1){ //Cria o FIFO entre o cliente e o servidor
		perror("mkfifo");
	}

	if(mkfifo("pipe_servidor_cliente",0666) == -1){ //Cria o FIFO entre o servidor e o cliente
		perror("mkfifo");
	}
	...
	if((fdr = open("pipe_servidor_cliente",O_WRONLY)) == -1){ //Abre o FIFO entre o servidor e o cliente para servir apenas de escrita
			...
			printf("Opened pipe server to client.\n");
		}
	...
	unlink("pipe_cliente_servidor"); //Corrige a obrigação de dar unlink à mão no final de cada execução
	return 0;
}	